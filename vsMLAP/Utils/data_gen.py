...
class Task_Generator(object):
    ...
    def get_data_loader(self, prm, meta_split='meta_train', limit_train_samples=None):
        ...
        # kwargs = {'num_workers': 0, 'pin_memory': True, 'num_samples': len(test_dataset)}
...
#            train_dataset.train_labels = np.array(train_dataset.train_labels)[sampled_inds]
        if isinstance(train_dataset.data, np.ndarray):
            ...
            train_dataset.targets = np.array(train_dataset.targets)[sampled_inds]
 
        else:
            sampled_inds = torch.randperm(n_train_samples_orig)[:limit_train_samples]
            train_dataset.data = train_dataset.data[sampled_inds]
            train_dataset.targets = train_dataset.targets[sampled_inds]
