...
from Utils.common import grad_step, get_loss_criterion, write_to_log, get_value, prior_grad_step, transfer_weights, prior_get_grad
...
def run_meta_iteration(i_iter, prior_model, task_generator, prm):
    ...
    if prior_optimizer.param_groups[0]['params'][0].grad is None:
        object.grad_init(prm, prior_model, loss_criterion, iter(mb_data_loaders[0]['train']), mb_data_loaders[0]['train'], prior_optimizer)
    # all_params = all_post_param + prior_params
    all_posterior_optimizers = [optim_func(filter(lambda p: p.requires_grad, posteriors_models[i].parameters()), optim_args) for i in range(meta_batch_size)]
    ...
    for i_inner_step in range(n_inner_steps):
        ...
        task_loss_list, info = get_objective(prior_model, prm, mb_data_loaders, object.feval,
                                             mb_iterators, posteriors_models, loss_criterion, meta_batch_size,
        ...
        for i_task in range(meta_batch_size):
            if isinstance(task_loss_list[i_task], int):
                continue
            grad_step(task_loss_list[i_task], posteriors_models[i_task], loss_criterion,
                      all_posterior_optimizers[i_task], prm,
                      mb_iterators[i_task], mb_data_loaders[i_task]['train'], lr_schedule, prm.lr, i_iter)
        ...
        prior_grad_step(prior_optimizer, prm.meta_batch_size, prm, prm.prior_lr_schedule, prm.prior_lr, i_iter)
        ...
        if (i_inner_step) % log_interval == 0:
            batch_acc = info['correct_count'] / info['sample_count']
            print(cmn.status_string(i_iter, prm.n_meta_train_epochs, i_inner_step, n_inner_steps, batch_acc) +
                  ' Empiric-Loss: {:.4f}'.
                  format(info['avg_empirical_loss']))
