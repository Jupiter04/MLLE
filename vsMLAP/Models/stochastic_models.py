...
def get_model(prm, model_type='Stochastic'):
    ...
    if model_name == 'FcNet3':
        ...
        model = AllCnn(model_type, model_name, linear_layer, conv2d_layer, task_info)
...
class FcNet3(general_model):
    def __init__(self, model_type, model_name, linear_layer, conv2d_layer, task_info):
        ...
        self.bn1 = nn.BatchNorm1d(n_hidden1, momentum=1, affine=True)
        self.fc2 = linear_layer(n_hidden1, n_hidden2)
        self.bn2 = nn.BatchNorm1d(n_hidden2, momentum=1, affine=True)
        self.fc3 = linear_layer(n_hidden2, n_hidden3)
        self.bn3 = nn.BatchNorm1d(n_hidden3, momentum=1, affine=True)
...
class AllCnn(general_model):
    def __init__(self, model_type, model_name, linear_layer, conv2d_layer, task_info, opt = {'d':0.5}, c1=96, c2= 192):
        ...
        def convbn(ci,co,ksz,s=1,pz=0):
            return nn.Sequential(
                conv2d_layer(ci,co,ksz,stride=s,padding=pz),
                nn.BatchNorm2d(co),
                nn.ReLU(True))
